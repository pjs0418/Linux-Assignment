...
static void pxt4_xattr_block_cache_insert(struct mb_cache *,
					  struct buffer_head *);
static struct buffer_head *
pxt4_xattr_block_cache_find(struct inode *, struct pxt4_xattr_header *,
...
#define EA_BLOCK_CACHE(inode)	(((struct pxt4_sb_info *) \
				inode->i_sb->s_fs_info)->s_ea_block_cache)

#define EA_INODE_CACHE(inode)	(((struct pxt4_sb_info *) \
...
void pxt4_xattr_inode_set_class(struct inode *ea_inode)
{
	lockdep_set_subclass(&ea_inode->i_rwsem, 1);
}
#endif

static __le32 pxt4_xattr_block_csum(struct inode *inode,
				    sector_t block_nr,
				    struct pxt4_xattr_header *hdr)
{
	...
	__u32 dummy_csum = 0;
	int offset = offsetof(struct pxt4_xattr_header, h_checksum);

	csum = pxt4_chksum(sbi, sbi->s_csum_seed, (__u8 *)&dsk_block_nr,
			   sizeof(dsk_block_nr));
...
static int pxt4_xattr_block_csum_verify(struct inode *inode,
					struct buffer_head *bh)
{
	struct pxt4_xattr_header *hdr = BHDR(bh);
	int ret = 1;

	if (pxt4_has_metadata_csum(inode->i_sb)) {
		lock_buffer(bh);
		ret = (hdr->h_checksum == pxt4_xattr_block_csum(inode,
							bh->b_blocknr, hdr));
		unlock_buffer(bh);
	}
	return ret;
...
static void pxt4_xattr_block_csum_set(struct inode *inode,
				      struct buffer_head *bh)
{
	if (pxt4_has_metadata_csum(inode->i_sb))
		BHDR(bh)->h_checksum = pxt4_xattr_block_csum(inode,
...
static int
pxt4_xattr_check_entries(struct pxt4_xattr_entry *entry, void *end,
			 void *value_start)
{
	struct pxt4_xattr_entry *e = entry;
...
static inline int
__pxt4_xattr_check_block(struct inode *inode, struct buffer_head *bh,
			 const char *function, unsigned int line)
{
	...
	error = -EFSBADCRC;
	if (!pxt4_xattr_block_csum_verify(inode, bh))
		goto errout;
	error = pxt4_xattr_check_entries(BFIRST(bh), bh->b_data + bh->b_size,
					 bh->b_data);
errout:
	if (error)
		__pxt4_error_inode(inode, function, line, 0,
...
#define pxt4_xattr_check_block(inode, bh) \
	__pxt4_xattr_check_block((inode), (bh),  __func__, __LINE__)


static int
__xattr_check_inode(struct inode *inode, struct pxt4_xattr_ibody_header *header,
...
static u32
pxt4_xattr_inode_hash(struct pxt4_sb_info *sbi, const void *buffer, size_t size)
{
	return pxt4_chksum(sbi, sbi->s_csum_seed, buffer, size);
}

static u64 pxt4_xattr_inode_get_ref(struct inode *ea_inode)
{
	return ((u64)ea_inode->i_ctime.tv_sec << 32) |
		(u32) inode_peek_iversion_raw(ea_inode);
}

static void pxt4_xattr_inode_set_ref(struct inode *ea_inode, u64 ref_count)
{
	ea_inode->i_ctime.tv_sec = (u32)(ref_count >> 32);
	inode_set_iversion_raw(ea_inode, ref_count & 0xffffffff);
}

static u32 pxt4_xattr_inode_get_hash(struct inode *ea_inode)
{
	return (u32)ea_inode->i_atime.tv_sec;
}

static void pxt4_xattr_inode_set_hash(struct inode *ea_inode, u32 hash)
{
 ...
 */
static int pxt4_xattr_inode_read(struct inode *ea_inode, void *buf, size_t size)
{
	...
	ret = pxt4_bread_batch(ea_inode, 0 /* block */, bh_count,
			       true /* wait */, bhs);
	if (ret)
		goto free_bhs;
				 ...
				 u32 ea_inode_hash, struct inode **ea_inode)
{
	...
	pxt4_xattr_inode_set_class(inode);
...
static int
pxt4_xattr_inode_verify_hashes(struct inode *ea_inode,
			       struct pxt4_xattr_entry *entry, void *buffer,
			       size_t size)
{
	u32 hash;
	...
	if (entry) {
		...
		e_hash = pxt4_xattr_hash_entry(entry->e_name, entry->e_name_len,
					       &tmp_data, 1);
 ...
 */
static int
pxt4_xattr_inode_get(struct inode *inode, struct pxt4_xattr_entry *entry,
		     void *buffer, size_t size)
{
	...
	err = pxt4_xattr_inode_iget(inode, le32_to_cpu(entry->e_value_inum),
	...
	err = pxt4_xattr_inode_read(ea_inode, buffer, size);
	if (err)
		goto out;
	...
	if (!pxt4_test_inode_state(ea_inode, PXT4_STATE_LUSTRE_EA_INODE)) {
		...
		if (ea_inode_cache)
			mb_cache_entry_create(ea_inode_cache, GFP_NOFS,
					pxt4_xattr_inode_get_hash(ea_inode),
		     ...
		     void *buffer, size_t buffer_size)
{
	...
	if (buffer) {
		...
		if (entry->e_value_inum) {
			error = pxt4_xattr_inode_get(inode, entry, buffer,
						     size);
			if (error)
				goto cleanup;
		} else {
...
static int
pxt4_xattr_list_entries(struct dentry *dentry, struct pxt4_xattr_entry *entry,
...
static int
pxt4_xattr_block_list(struct dentry *dentry, char *buffer, size_t buffer_size)
{
	...
	if (error)
		goto cleanup;
	pxt4_xattr_block_cache_insert(EA_BLOCK_CACHE(inode), bh);
	error = pxt4_xattr_list_entries(dentry, BFIRST(bh), buffer,
					buffer_size);
cleanup:
	brelse(bh);
	return error;
...
static int
pxt4_xattr_ibody_list(struct dentry *dentry, char *buffer, size_t buffer_size)
{
	struct inode *inode = d_inode(dentry);
	struct pxt4_xattr_ibody_header *header;
	struct pxt4_inode *raw_inode;
	...
	if (error)
		goto cleanup;
	error = pxt4_xattr_list_entries(dentry, IFIRST(header),
					buffer, buffer_size);
 ...
 */
ssize_t
pxt4_listxattr(struct dentry *dentry, char *buffer, size_t buffer_size)
{
	int ret, ret2;
	...
	ret = pxt4_xattr_block_list(dentry, buffer, buffer_size);
	if (ret < 0)
		goto errout;
	ret += ret2;
errout:
...
int pxt4_get_inode_usage(struct inode *inode, qsize_t *usage)
{
	...
	if (PXT4_I(inode)->i_file_acl) {
		...
		ret = pxt4_xattr_check_block(inode, bh);
		if (ret)
			goto out;
...
static int pxt4_xattr_inode_alloc_quota(struct inode *inode, size_t len)
{
	int err;
...
static void pxt4_xattr_inode_free_quota(struct inode *parent,
					struct inode *ea_inode,
					size_t len)
{
	if (ea_inode &&
...
int __pxt4_xattr_set_credits(struct super_block *sb, struct inode *inode,
			     ...
			     bool is_create)
{
	 ...
	 */
	if (inode && pxt4_has_inline_data(inode))
		credits += pxt4_writepage_trans_blocks(inode) + 1;
...
static int pxt4_xattr_ensure_credits(handle_t *handle, struct inode *inode,
				     ...
				     bool dirty, bool block_csum)
{
	...
	if (!error)
		return 0;
	if (error < 0) {
		pxt4_warning(inode->i_sb, "Extend journal (error %d)", error);
		return error;
	}

	if (bh && dirty) {
		if (block_csum)
			pxt4_xattr_block_csum_set(inode, bh);
		error = pxt4_handle_dirty_metadata(handle, NULL, bh);
		if (error) {
			pxt4_warning(inode->i_sb, "Handle metadata (error %d)",
				     error);
			return error;
		}
	}

	error = pxt4_journal_restart(handle, credits);
	if (error) {
		pxt4_warning(inode->i_sb, "Restart journal (error %d)", error);
		return error;
	}

	if (bh) {
		error = pxt4_journal_get_write_access(handle, bh);
		if (error) {
...
static int pxt4_xattr_inode_update_ref(handle_t *handle, struct inode *ea_inode,
				       int ref_change)
{
	...
	ret = pxt4_reserve_inode_write(handle, ea_inode, &iloc);
	if (ret)
		goto out;

	ref_count = pxt4_xattr_inode_get_ref(ea_inode);
	ref_count += ref_change;
	pxt4_xattr_inode_set_ref(ea_inode, ref_count);
	...
	if (ref_change > 0) {
		...
		if (ref_count == 1) {
			...
			pxt4_orphan_del(handle, ea_inode);

			if (ea_inode_cache) {
				hash = pxt4_xattr_inode_get_hash(ea_inode);
		...
		if (ref_count == 0) {
			...
			pxt4_orphan_add(handle, ea_inode);
	...
	ret = pxt4_mark_iloc_dirty(handle, ea_inode, &iloc);
	if (ret)
		pxt4_warning_inode(ea_inode,
				   "pxt4_mark_iloc_dirty() failed ret=%d", ret);
out:
	inode_unlock(ea_inode);
	return ret;
}

static int pxt4_xattr_inode_inc_ref(handle_t *handle, struct inode *ea_inode)
{
	return pxt4_xattr_inode_update_ref(handle, ea_inode, 1);
}

static int pxt4_xattr_inode_dec_ref(handle_t *handle, struct inode *ea_inode)
{
	return pxt4_xattr_inode_update_ref(handle, ea_inode, -1);
}

static int pxt4_xattr_inode_inc_ref_all(handle_t *handle, struct inode *parent,
					struct pxt4_xattr_entry *first)
{
	struct inode *ea_inode;
	struct pxt4_xattr_entry *entry;
	struct pxt4_xattr_entry *failed_entry;
	unsigned int ea_ino;
	int err, saved_err;
	...
	for (entry = first; !IS_LAST_ENTRY(entry);
		...
		err = pxt4_xattr_inode_iget(parent, ea_ino,
		...
		if (err) {
			pxt4_warning_inode(ea_inode, "inc ref error %d", err);
			iput(ea_inode);
			goto cleanup;
		}
		iput(ea_inode);
...
cleanup:
	...
	for (entry = first; entry != failed_entry;
		...
		err = pxt4_xattr_inode_iget(parent, ea_ino,
					    ...
					    &ea_inode);
		if (err) {
			pxt4_warning(parent->i_sb,
				     "cleanup ea_ino %u iget error %d", ea_ino,
				     err);
			continue;
		}
		err = pxt4_xattr_inode_dec_ref(handle, ea_inode);
		if (err)
			pxt4_warning_inode(ea_inode, "cleanup dec ref error %d",
					   err);
		iput(ea_inode);
	}
	return saved_err;
...
static void
pxt4_xattr_inode_dec_ref_all(handle_t *handle, struct inode *parent,
			     struct buffer_head *bh,
			     ...
			     int extra_credits, bool skip_quota)
{
	...
	for (entry = first; !IS_LAST_ENTRY(entry);
		...
		err = pxt4_xattr_ensure_credits(handle, parent, credits, bh,
						dirty, block_csum);
		if (err) {
			pxt4_warning_inode(ea_inode, "Ensure credits err=%d",
					   err);
			continue;
		}
		...
		err = pxt4_xattr_inode_dec_ref(handle, ea_inode);
		if (err) {
			pxt4_warning_inode(ea_inode, "ea_inode dec ref err=%d",
					   err);
			continue;
		}

		if (!skip_quota)
			pxt4_xattr_inode_free_quota(parent, ea_inode,
			 ...
			 int extra_credits)
{
	...
	if (ref == 1) {
		 ...
		 * pxt4_xattr_block_set() to reliably detect freed block
		 */
		if (ea_block_cache)
		...
		if (pxt4_has_feature_ea_inode(inode->i_sb))
			pxt4_xattr_inode_dec_ref_all(handle, inode, bh,
						     BFIRST(bh),
						     true /* block_csum */,
						     ea_inode_array,
						     extra_credits,
		 ...
		 * call pxt4_handle_dirty_metadata() with locked buffer as
 ...
 */
static int pxt4_xattr_inode_write(handle_t *handle, struct inode *ea_inode,
...
retry:
	while (ret >= 0 && ret < max_blocks) {
		...
		if (ret <= 0) {
			...
			if (ret == -ENOSPC &&
			    pxt4_should_retry_alloc(ea_inode->i_sb, &retries)) {
				ret = 0;
				goto retry;
			}
			break;
	...
	while (wsize < bufsize) {
		...
		csize = (bufsize - wsize) > blocksize ? blocksize :
								bufsize - wsize;
		bh = pxt4_getblk(handle, ea_inode, block, 0);
		if (IS_ERR(bh))
			return PTR_ERR(bh);
		if (!bh) {
			WARN_ON_ONCE(1);
		...
		pxt4_handle_dirty_metadata(handle, ea_inode, bh);
	...
	pxt4_update_i_disksize(ea_inode, wsize);
	inode_unlock(ea_inode);
 ...
 */
static struct inode *pxt4_xattr_inode_create(handle_t *handle,
					     struct inode *inode, u32 hash)
{
	...
	if (!IS_ERR(ea_inode)) {
		...
		pxt4_set_aops(ea_inode);
		pxt4_xattr_inode_set_class(ea_inode);
		unlock_new_inode(ea_inode);
		pxt4_xattr_inode_set_ref(ea_inode, 1);
		pxt4_xattr_inode_set_hash(ea_inode, hash);
		err = pxt4_mark_inode_dirty(handle, ea_inode);
		if (!err)
			err = pxt4_inode_attach_jinode(ea_inode);
		if (err) {
			iput(ea_inode);
			return ERR_PTR(err);
		}
...
static struct inode *
pxt4_xattr_inode_cache_find(struct inode *inode, const void *value,
			    size_t value_len, u32 hash)
{
	...
	WARN_ON_ONCE(pxt4_handle_valid(journal_current_handle()) &&
		     !(current->flags & PF_MEMALLOC_NOFS));

	ea_data = pxt4_kvmalloc(value_len, GFP_NOFS);
	if (!ea_data) {
 ...
 */
static int pxt4_xattr_inode_lookup_create(handle_t *handle, struct inode *inode,
					  ...
					  struct inode **ret_inode)
{
	...
	ea_inode = pxt4_xattr_inode_create(handle, inode, hash);
	if (IS_ERR(ea_inode))
		return PTR_ERR(ea_inode);

	err = pxt4_xattr_inode_write(handle, ea_inode, value, value_len);
	if (err) {
		pxt4_xattr_inode_dec_ref(handle, ea_inode);
		iput(ea_inode);
		return err;
	}
				...
				bool is_block)
{
	...
	if (!s->not_found && here->e_value_inum) {
		ret = pxt4_xattr_inode_iget(inode,
	...
	if (i->value && in_inode) {
		...
		ret = pxt4_xattr_inode_alloc_quota(inode, i->value_len);
		if (ret)
			goto out;

		ret = pxt4_xattr_inode_lookup_create(handle, inode, i->value,
						     i->value_len,
						     &new_ea_inode);
		if (ret) {
			new_ea_inode = NULL;
			pxt4_xattr_inode_free_quota(inode, NULL, i->value_len);
			goto out;
		}
	}
	...
	if (old_ea_inode) {
		...
		ret = pxt4_xattr_inode_dec_ref(handle, old_ea_inode);
		if (ret) {
			/* Release newly required ref count on new_ea_inode. */
			if (new_ea_inode) {
				int err;

				err = pxt4_xattr_inode_dec_ref(handle,
							       new_ea_inode);
				if (err)
					pxt4_warning_inode(new_ea_inode,
						  "dec ref new_ea_inode err=%d",
						  err);
				pxt4_xattr_inode_free_quota(inode, new_ea_inode,
							    i->value_len);
			}
			goto out;
		}

		pxt4_xattr_inode_free_quota(inode, old_ea_inode,
...
update_hash:
	if (i->value) {
		...
		if (in_inode) {
			...
			crc32c_hash = cpu_to_le32(
				       pxt4_xattr_inode_get_hash(new_ea_inode));
			hash = pxt4_xattr_hash_entry(here->e_name,
						     here->e_name_len,
						     &crc32c_hash, 1);
		} else if (is_block) {
	...
	if (is_block)
		pxt4_xattr_rehash((struct pxt4_xattr_header *)s->base);
...
struct pxt4_xattr_block_find {
	struct pxt4_xattr_search s;
	struct buffer_head *bh;
};

static int
pxt4_xattr_block_find(struct inode *inode, struct pxt4_xattr_info *i,
		      struct pxt4_xattr_block_find *bs)
{
	...
	if (PXT4_I(inode)->i_file_acl) {
		...
		error = pxt4_xattr_check_block(inode, bs->bh);
		if (error)
			return error;
...
static int
pxt4_xattr_block_set(handle_t *handle, struct inode *inode,
		     struct pxt4_xattr_info *i,
		     struct pxt4_xattr_block_find *bs)
{
	struct super_block *sb = inode->i_sb;
	struct buffer_head *new_bh = NULL;
	struct pxt4_xattr_search s_copy = bs->s;
	struct pxt4_xattr_search *s = &s_copy;
...
#define header(x) ((struct pxt4_xattr_header *)(x))

	if (s->base) {
		BUFFER_TRACE(bs->bh, "get_write_access");
		error = pxt4_journal_get_write_access(handle, bs->bh);
		if (error)
			goto cleanup;
		lock_buffer(bs->bh);
		...
		if (header(s->base)->h_refcount == cpu_to_le32(1)) {
			...
			/*
			 * This must happen under buffer lock for
			 * pxt4_xattr_block_set() to reliably detect modified
			 * block
			 */
			if (ea_block_cache)
				mb_cache_entry_delete(ea_block_cache, hash,
			...
			error = pxt4_xattr_set_entry(i, s, handle, inode,
						     true /* is_block */);
			pxt4_xattr_block_csum_set(inode, bs->bh);
			unlock_buffer(bs->bh);
			...
			if (!s->not_found && s->here->e_value_inum) {
				...
				error = pxt4_xattr_inode_iget(inode, ea_ino,
	...
	error = pxt4_xattr_set_entry(i, s, handle, inode, true /* is_block */);
	if (error == -EFSCORRUPTED)
		goto bad_block;
	if (error)
		goto cleanup;
...
inserted:
	if (!IS_LAST_ENTRY(s->first)) {
		new_bh = pxt4_xattr_block_cache_find(inode, header(s->base),
						     &ce);
		if (new_bh) {
			...
			if (new_bh == bs->bh)
				...
				error = pxt4_journal_get_write_access(handle,
								      new_bh);
				if (error)
					goto cleanup_dquot;
				lock_buffer(new_bh);
				...
				ea_bdebug(new_bh, "reusing; refcount now=%d",
					  ref);
				pxt4_xattr_block_csum_set(inode, new_bh);
				unlock_buffer(new_bh);
				error = pxt4_handle_dirty_metadata(handle,
								   inode,
								   new_bh);
			...
			pxt4_xattr_block_cache_insert(ea_block_cache, bs->bh);
			new_bh = bs->bh;
			get_bh(new_bh);
		} else {
			/* We need to allocate a new block */
			pxt4_fsblk_t goal, block;
...
getblk_failed:
			...
			lock_buffer(new_bh);
			error = pxt4_journal_get_create_access(handle, new_bh);
			if (error) {
				unlock_buffer(new_bh);
				error = -EIO;
				goto getblk_failed;
			...
			unlock_buffer(new_bh);
			pxt4_xattr_block_cache_insert(ea_block_cache, new_bh);
			error = pxt4_handle_dirty_metadata(handle, inode,
							   new_bh);
			if (error)
				goto cleanup;
		}
...
cleanup:
	if (ea_inode) {
		...
		error2 = pxt4_xattr_inode_dec_ref(handle, ea_inode);
		if (error2)
			pxt4_warning_inode(ea_inode, "dec ref error=%d",
					   error2);

		/* If there was an error, revert the quota charge. */
		if (error)
			pxt4_xattr_inode_free_quota(inode, ea_inode,
...
int pxt4_xattr_ibody_find(struct inode *inode, struct pxt4_xattr_info *i,
...
int pxt4_xattr_ibody_inline_set(handle_t *handle, struct inode *inode,
				struct pxt4_xattr_info *i,
				struct pxt4_xattr_ibody_find *is)
{
	struct pxt4_xattr_ibody_header *header;
...
static int pxt4_xattr_ibody_set(handle_t *handle, struct inode *inode,
				struct pxt4_xattr_info *i,
				struct pxt4_xattr_ibody_find *is)
{
	struct pxt4_xattr_ibody_header *header;
...
static int pxt4_xattr_value_same(struct pxt4_xattr_search *s,
				 struct pxt4_xattr_info *i)
{
	void *value;
...
static struct buffer_head *pxt4_xattr_get_block(struct inode *inode)
{
	struct buffer_head *bh;
	int error;
		      ...
		      int flags)
{
	...
	if (pxt4_handle_valid(handle)) {
		...
		credits = __pxt4_xattr_set_credits(inode->i_sb, inode, bh,
						   value_len,
						   flags & XATTR_CREATE);
		brelse(bh);

		if (!pxt4_handle_has_enough_credits(handle, credits)) {
			error = -ENOSPC;
			goto cleanup;
		}
	...
	error = pxt4_reserve_inode_write(handle, inode, &is.iloc);
	if (error)
		goto cleanup;
	...
	if (!value) {
		...
		if (!is.s.not_found && pxt4_xattr_value_same(&is.s, &i))
			goto cleanup;
		if (!bs.s.not_found && pxt4_xattr_value_same(&bs.s, &i))
			goto cleanup;
...
retry_inode:
		...
		if (!error && !bs.s.not_found) {
			...
			if (!error && !is.s.not_found) {
				i.value = NULL;
				error = pxt4_xattr_ibody_set(handle, inode, &i,
							     &is);
			} else if (error == -ENOSPC) {
				/*
...
int pxt4_xattr_set_credits(struct inode *inode, size_t value_len,
...
retry:
	error = pxt4_xattr_set_credits(inode, value_len, flags & XATTR_CREATE,
				       &credits);
	if (error)
		return error;
...
static void pxt4_xattr_shift_entries(struct pxt4_xattr_entry *entry,
				     ...
				     void *from, size_t n)
{
	struct pxt4_xattr_entry *last = entry;
	int new_offs;
...
static int pxt4_xattr_move_to_block(handle_t *handle, struct inode *inode,
				    ...
				    struct pxt4_xattr_entry *entry)
{
	struct pxt4_xattr_ibody_find *is = NULL;
	struct pxt4_xattr_block_find *bs = NULL;
	...
	is = kzalloc(sizeof(struct pxt4_xattr_ibody_find), GFP_NOFS);
	bs = kzalloc(sizeof(struct pxt4_xattr_block_find), GFP_NOFS);
	...
	if (entry->e_value_inum) {
		error = pxt4_xattr_inode_get(inode, entry, buffer, value_size);
		if (error)
			goto out;
	} else {
...
static int pxt4_xattr_make_inode_space(handle_t *handle, struct inode *inode,
				       struct pxt4_inode *raw_inode,
				       int isize_diff, size_t ifree,
				       size_t bfree, int *total_ino)
{
	struct pxt4_xattr_ibody_header *header = IHDR(inode, raw_inode);
	struct pxt4_xattr_entry *small_entry;
	struct pxt4_xattr_entry *entry;
	struct pxt4_xattr_entry *last;
...
retry:
	...
	ifree = pxt4_xattr_free_space(base, &min_offs, base, &total_ino);
	if (ifree >= isize_diff)
		goto shift;
	...
	if (PXT4_I(inode)->i_file_acl) {
		...
		min_offs = end - base;
		bfree = pxt4_xattr_free_space(BFIRST(bh), &min_offs, base,
					      NULL);
		brelse(bh);
	...
	error = pxt4_xattr_make_inode_space(handle, inode, raw_inode,
			...
			struct inode *inode)
{
	if (*ea_inode_array == NULL) {
		 ...
		 */
		(*ea_inode_array) =
			kmalloc(offsetof(struct pxt4_xattr_inode_array,
					 inodes[EIA_MASK]),
				GFP_NOFS);
		if (*ea_inode_array == NULL)
			return -ENOMEM;
		...
		new_array = kmalloc(
				offsetof(struct pxt4_xattr_inode_array,
					 inodes[count + EIA_INCR]),
				GFP_NOFS);
		if (new_array == NULL)
			return -ENOMEM;
		memcpy(new_array, *ea_inode_array,
		       offsetof(struct pxt4_xattr_inode_array, inodes[count]));
...
void pxt4_xattr_inode_array_free(struct pxt4_xattr_inode_array *ea_inode_array)
{
	int idx;
 ...
 */
static void
pxt4_xattr_block_cache_insert(struct mb_cache *ea_block_cache,
			      struct buffer_head *bh)
{
 ...
 */
static int
pxt4_xattr_cmp(struct pxt4_xattr_header *header1,
	       struct pxt4_xattr_header *header2)
{
	struct pxt4_xattr_entry *entry1, *entry2;
 ...
 */
static struct buffer_head *
pxt4_xattr_block_cache_find(struct inode *inode,
			    struct pxt4_xattr_header *header,
			    struct mb_cache_entry **pce)
{
	...
	while (ce) {
		...
		bh = pxt4_sb_bread(inode->i_sb, ce->e_value, REQ_PRIO);
		if (IS_ERR(bh)) {
...
struct mb_cache *
pxt4_xattr_create_cache(void)
{
	return mb_cache_create(HASH_BUCKET_BITS);
}

void pxt4_xattr_destroy_cache(struct mb_cache *cache)
{
	if (cache)
