...
static struct buffer_head *pxt4_append(handle_t *handle,
					struct inode *inode,
					pxt4_lblk_t *block)
{
...
static int pxt4_dx_csum_verify(struct inode *inode,
...
#define pxt4_read_dirblock(inode, block, type) \
	__pxt4_read_dirblock((inode), (block), (type), __func__, __LINE__)

static struct buffer_head *__pxt4_read_dirblock(struct inode *inode,
						pxt4_lblk_t block,
						...
						unsigned int line)
{
	...
	bh = pxt4_bread(NULL, inode, block, 0);
	if (IS_ERR(bh)) {
		__pxt4_warning(inode->i_sb, func, line,
	...
	if (is_dx(inode)) {
		if (block == 0)
			is_dx_block = 1;
		else if (pxt4_rec_len_from_disk(dirent->rec_len,
	...
	if (!is_dx_block) {
		if (pxt4_dirblock_csum_verify(inode, bh))
			set_buffer_verified(bh);
		else {
			pxt4_error_inode(inode, func, line, block,
...
static inline pxt4_lblk_t dx_get_block(struct dx_entry *entry);
static void dx_set_block(struct dx_entry *entry, pxt4_lblk_t value);
...
static struct dx_frame *dx_probe(struct pxt4_filename *fname,
				 struct inode *dir,
				 struct dx_hash_info *hinfo,
				 struct dx_frame *frame);
static void dx_release(struct dx_frame *frames);
static int dx_make_map(struct inode *dir, struct pxt4_dir_entry_2 *de,
		       unsigned blocksize, struct dx_hash_info *hinfo,
		       struct dx_map_entry map[]);
static void dx_sort_map(struct dx_map_entry *map, unsigned count);
static struct pxt4_dir_entry_2 *dx_move_dirents(char *from, char *to,
...
static int pxt4_htree_next_block(struct inode *dir, __u32 hash,
				 ...
				 __u32 *start_hash);
static struct buffer_head * pxt4_dx_find_entry(struct inode *dir,
...
static struct pxt4_dir_entry_tail *get_dirent_tail(struct inode *inode,
						   struct buffer_head *bh)
{
	struct pxt4_dir_entry_tail *t;

#ifdef PARANOID
	struct pxt4_dir_entry *d, *top;
...
static void __warn_no_space_for_csum(struct inode *inode, const char *func,
				     unsigned int line)
{
	__pxt4_warning_inode(inode, func, line,
		"No space for directory leaf checksum. Please run e2fsck -D.");
}

int pxt4_dirblock_csum_verify(struct inode *inode, struct buffer_head *bh)
{
	struct pxt4_dir_entry_tail *t;
	...
	if (t->det_checksum != pxt4_dirblock_csum(inode, bh->b_data,
						  (char *)t - bh->b_data))
		return 0;
...
static void pxt4_dirblock_csum_set(struct inode *inode,
				 struct buffer_head *bh)
{
	struct pxt4_dir_entry_tail *t;
	...
	t->det_checksum = pxt4_dirblock_csum(inode, bh->b_data,
					     (char *)t - bh->b_data);
}

int pxt4_handle_dirty_dirblock(handle_t *handle,
			       struct inode *inode,
			       struct buffer_head *bh)
{
	pxt4_dirblock_csum_set(inode, bh);
	return pxt4_handle_dirty_metadata(handle, inode, bh);
}
			   ...
			   int count_offset, int count, struct dx_tail *t)
{
	...
	csum = pxt4_chksum(sbi, ei->i_csum_seed, (__u8 *)dirent, size);
	csum = pxt4_chksum(sbi, csum, (__u8 *)t, offset);
			       ...
			       struct pxt4_dir_entry *dirent)
{
	...
	if (t->dt_checksum != pxt4_dx_csum(inode, dirent, count_offset,
					    count, t))
		return 0;
	return 1;
}

static void pxt4_dx_csum_set(struct inode *inode, struct pxt4_dir_entry *dirent)
{
	struct dx_countlimit *c;
	struct dx_tail *t;
	int count_offset, limit, count;
	...
	t->dt_checksum = pxt4_dx_csum(inode, dirent, count_offset, count, t);
}

static inline int pxt4_handle_dirty_dx_node(handle_t *handle,
					    struct inode *inode,
					    struct buffer_head *bh)
{
	pxt4_dx_csum_set(inode, (struct pxt4_dir_entry *)bh->b_data);
	return pxt4_handle_dirty_metadata(handle, inode, bh);
}
...
static inline void dx_set_block(struct dx_entry *entry, pxt4_lblk_t value)
{
	...
	while ((char *) de < base + size)
	{
		if (de->inode)
		{
			if (show_names)
			{
				...
				int len = de->name_len;
				char *name = de->name;
				pxt4fs_dirhash(dir, de->name, de->name_len, &h);
			     ...
			     struct dx_entry *entries, int levels)
{
	...
	for (i = 0; i < count; i++, entries++)
	{
		...
		pxt4_lblk_t hash  = i ? dx_get_hash(entries): 0;
		u32 range = i < count - 1? (dx_get_hash(entries + 1) - hash): ~hash;
		struct stats stats;
		printk("%s%3u:%03u hash %8x/%8x ",levels?"":"   ", i, block, hash, range);
		bh = pxt4_bread(NULL,dir, block, 0);
		if (!bh || IS_ERR(bh))
			continue;
		stats = levels?
		   dx_show_entries(hinfo, dir, ((struct dx_node *) bh->b_data)->entries, levels - 1):
		   dx_show_leaf(dir, hinfo, (struct pxt4_dir_entry_2 *)
...
dx_probe(struct pxt4_filename *fname, struct inode *dir,
	 struct dx_hash_info *hinfo, struct dx_frame *frame_in)
{
	...
	if (root->info.hash_version != DX_HASH_TEA &&
		...
		pxt4_warning_inode(dir, "Unrecognised inode hash code %u",
	...
	if (root->info.unused_flags & 1) {
		pxt4_warning_inode(dir, "Unimplemented hash flags: %#06x",
	...
	if (dx_get_limit(entries) != dx_root_limit(dir,
						   root->info.info_length)) {
		pxt4_warning_inode(dir, "dx entry: limit %u != root limit %u",
	...
	while (1) {
		...
		if (!indirect--)
			return frame;
		frame++;
		frame->bh = pxt4_read_dirblock(dir, dx_get_block(at), INDEX);
		if (IS_ERR(frame->bh)) {
				 ...
				 __u32 *start_hash)
{
	 ...
	 */
	while (num_frames--) {
		bh = pxt4_read_dirblock(dir, dx_get_block(p->at), INDEX);
		if (IS_ERR(bh))
			return PTR_ERR(bh);
		p++;
		brelse(p->bh);
...
{
	struct buffer_head *bh;
	struct pxt4_dir_entry_2 *de, *top;
	int err = 0, count = 0;
	struct fscrypt_str fname_crypto_str = FSTR_INIT(NULL, 0), tmp_str;

	dxtrace(printk(KERN_INFO "In htree dirblock_to_tree: block %lu\n",
							(unsigned long)block));
	bh = pxt4_read_dirblock(dir, block, DIRENT_HTREE);
	if (IS_ERR(bh))
		return PTR_ERR(bh);
	...
	for (; de < top; de = pxt4_next_entry(de, dir->i_sb->s_blocksize)) {
		...
		if (!IS_ENCRYPTED(dir)) {
			...
			err = pxt4_htree_store_dirent(dir_file,
			 ...
			 __u32 start_minor_hash, __u32 *next_hash)
{
	...
	if (!start_hash && !start_minor_hash) {
		...
		tmp_str.name = de->name;
		tmp_str.len = de->name_len;
		err = pxt4_htree_store_dirent(dir_file, 0, 0,
					      de, &tmp_str);
		if (err != 0)
			goto errout;
		count++;
...
static inline int search_dirblock(struct buffer_head *bh,
				  ...
				  struct pxt4_dir_entry_2 **res_dir)
{
	return pxt4_search_dir(bh, bh->b_data, dir->i_sb->s_blocksize, dir,
...
static void dx_insert_block(struct dx_frame *frame, u32 hash, pxt4_lblk_t block)
{
...
void pxt4_fname_setup_ci_filename(struct inode *dir, const struct qstr *iname,
 ...
 */
static inline bool pxt4_match(const struct inode *parent,
			      const struct pxt4_filename *fname,
			      const struct pxt4_dir_entry_2 *de)
{
	...
	if (PXT4_SB(parent->i_sb)->s_encoding && IS_CASEFOLDED(parent)) {
		if (fname->cf_name.name) {
			...
			return !pxt4_ci_compare(parent, &cf, &entry, true);
		}
		return !pxt4_ci_compare(parent, fname->usr_fname, &entry,
					false);
	}
#endif
 ...
 */
int pxt4_search_dir(struct buffer_head *bh, char *search_buf, int buf_size,
		    struct inode *dir, struct pxt4_filename *fname,
		    unsigned int offset, struct pxt4_dir_entry_2 **res_dir)
{
	struct pxt4_dir_entry_2 * de;
	char * dlimit;
	int de_len;
	...
	while ((char *) de < dlimit) {
		...
		if ((char *) de + de->name_len <= dlimit &&
			 ...
			 * a full check */
			if (pxt4_check_dir_entry(dir, NULL, de, bh, search_buf,
						 buf_size, offset))
				return -1;
			*res_dir = de;
			return 1;
		...
		de_len = pxt4_rec_len_from_disk(de->rec_len,
						dir->i_sb->s_blocksize);
		if (de_len <= 0)
			return -1;
		offset += de_len;
...
static int is_dx_internal_node(struct inode *dir, pxt4_lblk_t block,
			       struct pxt4_dir_entry *de)
{
	...
	if (block == 0)
		return 1;
	if (de->inode == 0 &&
	    pxt4_rec_len_from_disk(de->rec_len, sb->s_blocksize) ==
			sb->s_blocksize)
		return 1;
	return 0;
}
					     ...
					     int *inlined)
{
	...
	*res_dir = NULL;
	...
	if (pxt4_has_inline_data(dir)) {
		int has_inline_data = 1;
		ret = pxt4_find_inline_entry(dir, fname, res_dir,
					     &has_inline_data);
		if (has_inline_data) {
			if (inlined)
				*inlined = 1;
	...
	if (is_dx(dir)) {
		ret = pxt4_dx_find_entry(dir, fname, res_dir);
		/*
...
restart:
	do {
		...
		if (ra_ptr >= ra_max) {
			...
			retval = pxt4_bread_batch(dir, block, ra_max,
						  false /* wait */, bh_use);
			if (retval) {
				ret = ERR_PTR(retval);
				ra_max = 0;
...
static struct buffer_head *pxt4_find_entry(struct inode *dir,
					   ...
					   int *inlined)
{
	...
	err = pxt4_fname_setup_filename(dir, d_name, 1, &fname);
	if (err == -ENOENT)
		return NULL;
	if (err)
		return ERR_PTR(err);

	bh = __pxt4_find_entry(dir, &fname, res_dir, inlined);

	pxt4_fname_free_filename(&fname);
	return bh;
}

static struct buffer_head *pxt4_lookup_entry(struct inode *dir,
					     ...
					     struct pxt4_dir_entry_2 **res_dir)
{
	...
	err = pxt4_fname_prepare_lookup(dir, dentry, &fname);
	if (err == -ENOENT)
		return NULL;
	if (err)
		return ERR_PTR(err);

	bh = __pxt4_find_entry(dir, &fname, res_dir, NULL);
...
static struct dentry *pxt4_lookup(struct inode *dir, struct dentry *dentry, unsigned int flags)
{
	struct inode *inode;
	struct pxt4_dir_entry_2 *de;
	struct buffer_head *bh;
...
struct dentry *pxt4_get_parent(struct dentry *child)
{
	__u32 ino;
	...
	bh = pxt4_find_entry(d_inode(child), &dotdot, &de, NULL);
	if (IS_ERR(bh))
		return ERR_CAST(bh);
	if (!bh)
 ...
 */
static struct pxt4_dir_entry_2 *
		...
		unsigned blocksize)
{
	...
	return (struct pxt4_dir_entry_2 *) (to - rec_len);
}
 ...
 */
static struct pxt4_dir_entry_2 *do_split(handle_t *handle, struct inode *dir,
			...
			struct dx_hash_info *hinfo)
{
	...
	struct pxt4_dir_entry_2 *de = NULL, *de2;
	int	csum_size = 0;
	int	err = 0, i;
	...
	bh2 = pxt4_append(handle, dir, &newblock);
	if (IS_ERR(bh2)) {
		brelse(*bh);
		*bh = NULL;
		return (struct pxt4_dir_entry_2 *) bh2;
	}
	...
	err = pxt4_journal_get_write_access(handle, frame->bh);
	if (err)
		goto journal_error;
	...
	count = dx_make_map(dir, (struct pxt4_dir_entry_2 *) data1,
			     blocksize, hinfo, map);
	map -= count;
	...
	de->rec_len = pxt4_rec_len_to_disk(data1 + (blocksize - csum_size) -
					   (char *) de,
					   blocksize);
	de2->rec_len = pxt4_rec_len_to_disk(data2 + (blocksize - csum_size) -
					    (char *) de2,
					    blocksize);
	if (csum_size) {
		pxt4_initialize_dirent_tail(*bh, blocksize);
		pxt4_initialize_dirent_tail(bh2, blocksize);
	}

	dxtrace(dx_show_leaf(dir, hinfo, (struct pxt4_dir_entry_2 *) data1,
			blocksize, 1));
	dxtrace(dx_show_leaf(dir, hinfo, (struct pxt4_dir_entry_2 *) data2,
			blocksize, 1));
	...
	err = pxt4_handle_dirty_dirblock(handle, dir, bh2);
	if (err)
		goto journal_error;
...
int pxt4_find_dest_de(struct inode *dir, struct inode *inode,
		      struct buffer_head *bh,
		      ...
		      struct pxt4_dir_entry_2 **dest_de)
{
	...
	de = (struct pxt4_dir_entry_2 *)buf;
	...
	while ((char *) de <= top) {
		...
		if ((de->inode ? rlen - nlen : rlen) >= reclen)
			break;
		de = (struct pxt4_dir_entry_2 *)((char *)de + rlen);
		offset += rlen;
	}
	if ((char *) de > top)
		return -ENOSPC;
			...
			struct pxt4_filename *fname)
{
	...
	if (de->inode) {
		struct pxt4_dir_entry_2 *de1 =
			(struct pxt4_dir_entry_2 *)((char *)de + nlen);
		de1->rec_len = pxt4_rec_len_to_disk(rlen - nlen, buf_size);
...
static int add_dirent_to_buf(handle_t *handle, struct pxt4_filename *fname,
			     struct inode *dir,
			     struct inode *inode, struct pxt4_dir_entry_2 *de,
			     struct buffer_head *bh)
{
	...
	if (!de) {
		err = pxt4_find_dest_de(dir, inode, bh, bh->b_data,
	...
	pxt4_insert_dentry(inode, de, blocksize, fname);
	...
	err = pxt4_handle_dirty_dirblock(handle, dir, bh);
	if (err)
		pxt4_std_error(dir->i_sb, err);
	return 0;
}
			    ...
			    struct inode *inode, struct buffer_head *bh)
{
	...
	if (retval) {
		pxt4_std_error(dir->i_sb, retval);
		brelse(bh);
		return retval;
	}
	...
	bh2 = pxt4_append(handle, dir, &block);
	if (IS_ERR(bh2)) {
		brelse(bh);
		return PTR_ERR(bh2);
	}
	...
	retval = pxt4_handle_dirty_dx_node(handle, dir, frame->bh);
	if (retval)
		goto out_frames;	
	retval = pxt4_handle_dirty_dirblock(handle, dir, bh2);
	if (retval)
		goto out_frames;	
 ...
 * semantics as pxt4_find_entry(). It returns NULL if it failed.
 *
 * NOTE!! The inode part of 'de' is left at 0 - which means you
 * may not sleep between calling this and putting something into
 * the entry, as someone else might have used it while you slept.
 */
static int pxt4_add_entry(handle_t *handle, struct dentry *dentry,
			  struct inode *inode)
{
...
#ifdef CONFIG_UNICODE
	if (pxt4_has_strict_mode(sbi) && IS_CASEFOLDED(dir) &&
	    sbi->s_encoding && utf8_validate(sbi->s_encoding, &dentry->d_name))
		return -EINVAL;
#endif

	retval = pxt4_fname_setup_filename(dir, &dentry->d_name, 0, &fname);
	if (retval)
		return retval;

	if (pxt4_has_inline_data(dir)) {
		retval = pxt4_try_add_inline_entry(handle, &fname, dir, inode);
		if (retval < 0)
			goto out;
		if (retval == 1) {
			retval = 0;
	...
	if (is_dx(dir)) {
		retval = pxt4_dx_add_entry(handle, &fname, dir, inode);
	...
	bh = pxt4_append(handle, dir, &block);
add_to_new_block:
	if (IS_ERR(bh)) {
		retval = PTR_ERR(bh);
		bh = NULL;
	...
	de = (struct pxt4_dir_entry_2 *) bh->b_data;
	de->inode = 0;
	de->rec_len = pxt4_rec_len_to_disk(blocksize - csum_size, blocksize);
...
again:
	...
	at = frame->at;
	bh = pxt4_read_dirblock(dir, dx_get_block(frame->at), DIRENT_HTREE);
	if (IS_ERR(bh)) {
		err = PTR_ERR(bh);
		bh = NULL;
		goto cleanup;
	...
	if (dx_get_count(entries) == dx_get_limit(entries)) {
		...
		node2->fake.rec_len = pxt4_rec_len_to_disk(sb->s_blocksize,
							   sb->s_blocksize);
		BUFFER_TRACE(frame->bh, "get_write_access");
		err = pxt4_journal_get_write_access(handle, frame->bh);
		if (err)
		...
		if (!add_level) {
			...
			if (err)
				goto journal_error;
			brelse (bh2);
			err = pxt4_handle_dirty_dx_node(handle, dir,
						   (frame - 1)->bh);
			if (err)
				goto journal_error;
			err = pxt4_handle_dirty_dx_node(handle, dir,
...
journal_error:
	pxt4_std_error(dir->i_sb, err); /* this is a no-op if err == 0 */
cleanup:
	brelse(bh);
	dx_release(frames);
...
/*
 * pxt4_generic_delete_entry deletes a directory entry by merging it
 * with the previous entry
 */
int pxt4_generic_delete_entry(handle_t *handle,
			      struct inode *dir,
			      ...
			      int csum_size)
{
	struct pxt4_dir_entry_2 *de, *pde;
	unsigned int blocksize = dir->i_sb->s_blocksize;
	int i;

	i = 0;
	pde = NULL;
	de = (struct pxt4_dir_entry_2 *)entry_buf;
	while (i < buf_size - csum_size) {
		...
		if (de == de_del)  {
			if (pde)
				pde->rec_len = pxt4_rec_len_to_disk(
					pxt4_rec_len_from_disk(pde->rec_len,
							       blocksize) +
					pxt4_rec_len_from_disk(de->rec_len,
							       blocksize),
					blocksize);
			else
				de->inode = 0;
...
static int pxt4_delete_entry(handle_t *handle,
			     struct inode *dir,
			     struct pxt4_dir_entry_2 *de_del,
			     struct buffer_head *bh)
{
	...
	if (pxt4_has_inline_data(dir)) {
		int has_inline_data = 1;
		err = pxt4_delete_inline_entry(handle, dir, de_del, bh,
					       &has_inline_data);
		if (has_inline_data)
			return err;
	}
	...
	err = pxt4_generic_delete_entry(handle, dir, de_del,
					bh, bh->b_data,
 ...
 */
static void pxt4_inc_count(handle_t *handle, struct inode *inode)
{
	inc_nlink(inode);
	if (is_dx(inode) &&
...
static int pxt4_add_nondir(handle_t *handle,
		struct dentry *dentry, struct inode *inode)
{
	int err = pxt4_add_entry(handle, dentry, inode);
	if (!err) {
...
static int pxt4_mknod(struct inode *dir, struct dentry *dentry,
...
static int pxt4_tmpfile(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	handle_t *handle;
	struct inode *inode;
	int err, retries = 0;
...
struct pxt4_dir_entry_2 *pxt4_init_dot_dotdot(struct inode *inode,
			  struct pxt4_dir_entry_2 *de,
			     ...
			     struct inode *inode)
{
	...
	inode->i_size = 0;
	dir_block = pxt4_append(handle, inode, &block);
	if (IS_ERR(dir_block))
		return PTR_ERR(dir_block);
	de = (struct pxt4_dir_entry_2 *)dir_block->b_data;
	pxt4_init_dot_dotdot(inode, de, blocksize, csum_size, dir->i_ino, 0);
	set_nlink(inode, 2);
	if (csum_size)
		pxt4_initialize_dirent_tail(dir_block, blocksize);

	BUFFER_TRACE(dir_block, "call pxt4_handle_dirty_metadata");
...
static int pxt4_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	handle_t *handle;
	struct inode *inode;
	int err, credits, retries = 0;
...
retry:
	...
	err = pxt4_init_new_dir(handle, dir, inode);
	if (err)
		goto out_clear_inode;
	err = pxt4_mark_inode_dirty(handle, inode);
	if (!err)
		err = pxt4_add_entry(handle, dentry, inode);
	if (err) {
out_clear_inode:
		clear_nlink(inode);
		unlock_new_inode(inode);
 ...
 */
bool pxt4_empty_dir(struct inode *inode)
{
	unsigned int offset;
	struct buffer_head *bh;
	struct pxt4_dir_entry_2 *de;
	 ...
	 */
	bh = pxt4_read_dirblock(inode, 0, DIRENT_HTREE);
	if (IS_ERR(bh))
		return true;
	...
	if (pxt4_check_dir_entry(inode, NULL, de, bh, bh->b_data, bh->b_size,
		...
		pxt4_warning_inode(inode, "directory missing '.'");
		brelse(bh);
		return true;
	}
	...
	while (offset < inode->i_size) {
		...
		if (pxt4_check_dir_entry(inode, NULL, de, bh,
...
static int pxt4_rmdir(struct inode *dir, struct dentry *dentry)
{
	int retval;
	struct inode *inode;
	struct buffer_head *bh;
	...
	retval = -ENOENT;
	bh = pxt4_find_entry(dir, &dentry->d_name, &de, NULL);
	if (IS_ERR(bh))
		return PTR_ERR(bh);
	if (!bh)
		goto end_rmdir;
	 ...
	 * negative dentries at pxt4_lookup(), when it is better
...
static int pxt4_unlink(struct inode *dir, struct dentry *dentry)
{
	int retval;
	struct inode *inode;
	struct buffer_head *bh;
	...
	pxt4_mark_inode_dirty(handle, dir);
	if (inode->i_nlink == 0)
		pxt4_warning_inode(inode, "Deleting file '%.*s' with no links",
				   dentry->d_name.len, dentry->d_name.name);
	else
		drop_nlink(inode);
	if (!inode->i_nlink)
...
static int pxt4_symlink(struct inode *dir,
			struct dentry *dentry, const char *symname)
{
	...
	if ((disk_link.len > PXT4_N_BLOCKS * 4)) {
		 ...
		 * because it calls into pxt4_write_begin() which can wait
...
static int pxt4_link(struct dentry *old_dentry,
 ...
 */
static struct buffer_head *pxt4_get_first_dir_block(handle_t *handle,
					struct inode *inode,
					int *retval,
					...
					int *inlined)
{
	...
	*inlined = 1;
	return pxt4_get_first_inline_block(inode, parent_de, retval);
}

struct pxt4_renament {
	struct inode *dir;
	struct dentry *dentry;
	struct inode *inode;
	bool is_dir;
...
static int pxt4_rename_dir_prepare(handle_t *handle, struct pxt4_renament *ent)
{
	int retval;

	ent->dir_bh = pxt4_get_first_dir_block(handle, ent->inode,
	...
	return pxt4_journal_get_write_access(handle, ent->dir_bh);
}

static int pxt4_rename_dir_finish(handle_t *handle, struct pxt4_renament *ent,
				  unsigned dir_ino)
{
	int retval;

	ent->parent_de->inode = cpu_to_le32(dir_ino);
	BUFFER_TRACE(ent->dir_bh, "call pxt4_handle_dirty_metadata");
	if (!ent->dir_inlined) {
		if (is_dx(ent->inode)) {
			retval = pxt4_handle_dirty_dx_node(handle,
							   ent->inode,
							   ent->dir_bh);
		} else {
			retval = pxt4_handle_dirty_dirblock(handle, ent->inode,
							    ent->dir_bh);
		}
	} else {
		retval = pxt4_mark_inode_dirty(handle, ent->inode);
	}
	if (retval) {
		pxt4_std_error(ent->dir->i_sb, retval);
		return retval;
	}
	return 0;
}

static int pxt4_setent(handle_t *handle, struct pxt4_renament *ent,
		       unsigned ino, unsigned file_type)
{
	int retval;

	BUFFER_TRACE(ent->bh, "get write access");
	retval = pxt4_journal_get_write_access(handle, ent->bh);
	if (retval)
		return retval;
	ent->de->inode = cpu_to_le32(ino);
	if (pxt4_has_feature_filetype(ent->dir->i_sb))
		ent->de->file_type = file_type;
	inode_inc_iversion(ent->dir);
	ent->dir->i_ctime = ent->dir->i_mtime =
		current_time(ent->dir);
	pxt4_mark_inode_dirty(handle, ent->dir);
	BUFFER_TRACE(ent->bh, "call pxt4_handle_dirty_metadata");
	if (!ent->inlined) {
		retval = pxt4_handle_dirty_dirblock(handle, ent->dir, ent->bh);
		if (unlikely(retval)) {
			pxt4_std_error(ent->dir->i_sb, retval);
			return retval;
		}
...
static void pxt4_resetent(handle_t *handle, struct pxt4_renament *ent,
			  unsigned ino, unsigned file_type)
{
	struct pxt4_renament old = *ent;
	int retval = 0;
	 ...
	 */
	old.bh = pxt4_find_entry(old.dir, &old.dentry->d_name, &old.de, NULL);
	if (IS_ERR(old.bh))
		retval = PTR_ERR(old.bh);
	if (!old.bh)
		retval = -ENOENT;
	if (retval) {
		pxt4_std_error(old.dir->i_sb, retval);
		return;
	}

	pxt4_setent(handle, &old, ino, file_type);
	brelse(old.bh);
}

static int pxt4_find_delete_entry(handle_t *handle, struct inode *dir,
				  const struct qstr *d_name)
{
	int retval = -ENOENT;
	struct buffer_head *bh;
	...
	bh = pxt4_find_entry(dir, d_name, &de, NULL);
	if (IS_ERR(bh))
		return PTR_ERR(bh);
	if (bh) {
		retval = pxt4_delete_entry(handle, dir, de, bh);
...
static void pxt4_rename_delete(handle_t *handle, struct pxt4_renament *ent,
			       int force_reread)
{
	int retval;
	/*
	...
	if (le32_to_cpu(ent->de->inode) != ent->inode->i_ino ||
	    ...
	    strncmp(ent->de->name, ent->dentry->d_name.name,
		    ent->de->name_len) ||
	    force_reread) {
		retval = pxt4_find_delete_entry(handle, ent->dir,
						&ent->dentry->d_name);
	} else {
		retval = pxt4_delete_entry(handle, ent->dir, ent->de, ent->bh);
		if (retval == -ENOENT) {
			retval = pxt4_find_delete_entry(handle, ent->dir,
							&ent->dentry->d_name);
		}
	...
	if (retval) {
		pxt4_warning_inode(ent->dir,
				   "Deleting old file: nlink %d, error=%d",
				   ent->dir->i_nlink, retval);
	}
}

static void pxt4_update_dir_count(handle_t *handle, struct pxt4_renament *ent)
{
	if (ent->dir_nlink_delta) {
		if (ent->dir_nlink_delta == -1)
			pxt4_dec_count(handle, ent->dir);
		else
			pxt4_inc_count(handle, ent->dir);
		pxt4_mark_inode_dirty(handle, ent->dir);
	}
}

static struct inode *pxt4_whiteout_for_rename(struct pxt4_renament *ent,
...
retry:
	...
	if (IS_ERR(wh)) {
		...
		if (PTR_ERR(wh) == -ENOSPC &&
		    pxt4_should_retry_alloc(ent->dir->i_sb, &retries))
			goto retry;
	} else {
		*h = handle;
		init_special_inode(wh, wh->i_mode, WHITEOUT_DEV);
		wh->i_op = &pxt4_special_inode_operations;
	}
	return wh;
}
 ...
 */
static int pxt4_rename(struct inode *old_dir, struct dentry *old_dentry,
		       struct inode *new_dir, struct dentry *new_dentry,
		       unsigned int flags)
{
	handle_t *handle = NULL;
	struct pxt4_renament old = {
		.dir = old_dir,
		.dentry = old_dentry,
		.inode = d_inode(old_dentry),
	};
	struct pxt4_renament new = {
		.dir = new_dir,
		.dentry = new_dentry,
	...
	new.bh = pxt4_find_entry(new.dir, &new.dentry->d_name,
				 &new.de, &new.inlined);
	if (IS_ERR(new.bh)) {
	...
	if (!(flags & RENAME_WHITEOUT)) {
		...
		whiteout = pxt4_whiteout_for_rename(&old, credits, &handle);
		if (IS_ERR(whiteout)) {
	...
	if (!new.bh) {
		retval = pxt4_add_entry(handle, new.dentry, old.inode);
		if (retval)
			goto end_rename;
	} else {
		retval = pxt4_setent(handle, &new,
	...
	pxt4_mark_inode_dirty(handle, old.inode);

	if (!whiteout) {
		/*
		 * ok, that's it
		 */
		pxt4_rename_delete(handle, &old, force_reread);
	}

	if (new.inode) {
		pxt4_dec_count(handle, new.inode);
		new.inode->i_ctime = current_time(new.inode);
	}
	old.dir->i_ctime = old.dir->i_mtime = current_time(old.dir);
	pxt4_update_dx_flag(old.dir);
	if (old.dir_bh) {
		retval = pxt4_rename_dir_finish(handle, &old, new.dir->i_ino);
		if (retval)
			goto end_rename;

		pxt4_dec_count(handle, old.dir);
		if (new.inode) {
			/* checked pxt4_empty_dir above, can't have another
			 * parent, pxt4_dec_count() won't work for many-linked
			 * dirs */
			clear_nlink(new.inode);
		} else {
			pxt4_inc_count(handle, new.dir);
			pxt4_update_dx_flag(new.dir);
			pxt4_mark_inode_dirty(handle, new.dir);
		}
	}
	pxt4_mark_inode_dirty(handle, old.dir);
	if (new.inode) {
		pxt4_mark_inode_dirty(handle, new.inode);
		if (!new.inode->i_nlink)
			pxt4_orphan_add(handle, new.inode);
	}
	retval = 0;
...
end_rename:
	if (whiteout) {
		if (retval) {
			pxt4_resetent(handle, &old,
				      old.inode->i_ino, old_file_type);
			drop_nlink(whiteout);
			pxt4_orphan_add(handle, whiteout);
		}
...
static int pxt4_cross_rename(struct inode *old_dir, struct dentry *old_dentry,
			     struct inode *new_dir, struct dentry *new_dentry)
{
	handle_t *handle = NULL;
	struct pxt4_renament old = {
	...
	old.bh = pxt4_find_entry(old.dir, &old.dentry->d_name,
				 &old.de, &old.inlined);
	if (IS_ERR(old.bh))
		return PTR_ERR(old.bh);
	/*
	...
	if (S_ISDIR(new.inode->i_mode)) {
		new.is_dir = true;
		retval = pxt4_rename_dir_prepare(handle, &new);
		if (retval)
			goto end_rename;
	}
	...
	retval = pxt4_setent(handle, &new, old.inode->i_ino, old.de->file_type);
	if (retval)
		goto end_rename;

	retval = pxt4_setent(handle, &old, new.inode->i_ino, new_file_type);
	if (retval)
		goto end_rename;
	...
	if (new.dir_bh) {
		retval = pxt4_rename_dir_finish(handle, &new, old.dir->i_ino);
		if (retval)
			goto end_rename;
	}
	pxt4_update_dir_count(handle, &old);
	pxt4_update_dir_count(handle, &new);
	retval = 0;
...
static int pxt4_rename2(struct inode *old_dir, struct dentry *old_dentry,
			struct inode *new_dir, struct dentry *new_dentry,
			unsigned int flags)
{
	int err;
	...
	if (flags & RENAME_EXCHANGE) {
		return pxt4_cross_rename(old_dir, old_dentry,
					 new_dir, new_dentry);
	}

	return pxt4_rename(old_dir, old_dentry, new_dir, new_dentry, flags);
}

/*
 * directories can handle most operations...
 */
const struct inode_operations pxt4_dir_inode_operations = {
	.create		= pxt4_create,
	.lookup		= pxt4_lookup,
	.link		= pxt4_link,
	.unlink		= pxt4_unlink,
	.symlink	= pxt4_symlink,
	.mkdir		= pxt4_mkdir,
	.rmdir		= pxt4_rmdir,
	.mknod		= pxt4_mknod,
	.tmpfile	= pxt4_tmpfile,
	.rename		= pxt4_rename2,
...
const struct inode_operations pxt4_special_inode_operations = {
	.setattr	= pxt4_setattr,
	.getattr	= pxt4_getattr,
	.listxattr	= pxt4_listxattr,
	.get_acl	= pxt4_get_acl,
