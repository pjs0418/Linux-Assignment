				   ...
				   struct inode *inode, unsigned len)
{
	...
	error = pxt4_xattr_ibody_inline_set(handle, inode, &i, &is);
	if (error) {
		if (error == -ENOSPC)
			       ...
			       unsigned copied, struct page *page)
{
	...
	kaddr = kmap_atomic(page);
	pxt4_write_inline_data(inode, &iloc, kaddr, pos, len);
	kunmap_atomic(kaddr);
	SetPageUptodate(page);
...
{
	pxt4_create_inline_data(handle, inode, inline_size);
...
int pxt4_inline_data_truncate(struct inode *inode, int *has_inline)
{
	...
	if (i_size < inline_size) {
		...
		if (inline_size > PXT4_MIN_INLINE_DATA_SIZE) {
			...
			err = pxt4_xattr_ibody_inline_set(handle, inode,
							  &i, &is);
			if (err)
				goto out_error;
		}
